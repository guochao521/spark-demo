...
//    spark.sql(" SELECT array_sort(array('aa', 'bb','b', 'd', null, 'c', 'a'));").show()//数组排序
//    spark.sql(" SELECT array_union(array(1, 2, 3), array(1, 3, 5));").show()//数组合并去除重复元素
//    spark.sql(" SELECT arrays_zip(array(1, 2, 3), array(2, 3, 4));").show(false)//数据合并
...
//    spark.sql(" SELECT slice(array(1, 2, 3, 4), -1, 2);").show()//数组截取
//
//    //spark内置函数示例-map操作
//    spark.sql("SELECT map_concat(map(1, 'a', 2, 'b'), map(3, 'c')); ").show(false)//将2个map进行合并
...
//    spark.sql(" SELECT current_timestamp();").show(false)//获取当前时间戳
...
//    spark.sql("  SELECT min(col) FROM VALUES (10), (-1), (NULL), (20) AS tab(col);").show()
...
//
    ...
    def name_add(name: String, sex:String): Unit = {
      name + "_" + sex
    }
    ...
    spark.udf.register("name_add", (name:String, sex:String) => {
      name + "_" + sex
    })
    spark.sql("select name, name_add(name, 'F') name_add, age from people").show()
